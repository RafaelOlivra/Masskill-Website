    ...
    return (
        <div className={styles['video-holder']} onClick={(e) => onClick ? onClick(e, { url, title }) : ''}>
            <div className={styles['responsive-video'] + ' ' + (onClick ? styles['dynamic'] : '')}>
                <iframe className={styles['video-iframe']} width="560" height="315" src={url} title={title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen loading="lazy"></iframe>
            </div>
        </div>
    )
}
